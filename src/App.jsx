...
function App() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-blue-50/50 flex flex-col font-sans text-slate-900 relative">
      ...
      {/* Modern Header */}
      <header className="sticky top-0 z-40 bg-brand-secondary/95 backdrop-blur-xl px-4 md:px-12 py-5 flex items-center justify-between shadow-2xl border-b border-white/10">
        <div className="flex items-center gap-4">
          <div className="bg-white p-3 rounded-2xl shadow-lg shadow-black/20 rotate-3">
            <Store size={28} className="text-brand-primary" />
          </div>
          <div>
            <h1 className="text-2xl font-black text-white tracking-tight">BODEGA <span className="text-brand-primary">JIVA</span></h1>
            <p className="text-[10px] text-blue-200 uppercase font-black tracking-[0.2em] mt-0.5">Tienda de Confianza</p>
          </div>
        </div>

        <button
          onClick={() => setIsCartOpen(true)}
          className="relative group p-4 bg-white/10 hover:bg-white/20 rounded-2xl border border-white/20 transition-all duration-300 active:scale-90"
        >
          <ShoppingCart className="text-white" size={26} />
          {totalItems > 0 && (
            <span className="absolute -top-2 -right-2 bg-brand-primary text-white text-[11px] font-black w-7 h-7 flex items-center justify-center rounded-full border-4 border-brand-secondary shadow-xl">
              {totalItems}
            </span>
          )}
        </button>
      ...
      <main className="flex-1">
        ...
        <section className="relative py-20 px-4 md:px-12 bg-brand-secondary overflow-hidden">
          ...
          <div className="relative max-w-7xl mx-auto flex flex-col lg:flex-row items-center gap-12">
            <div className="flex-1 text-center lg:text-left space-y-8">
              ...
              <p className="text-lg text-blue-100/80 max-w-xl font-medium leading-relaxed mx-auto lg:mx-0">
                Todo lo que necesitas, helado y en tu puerta. La mejor selección de bebidas con delivery express.
              </p>
              <div className="flex flex-wrap gap-4 justify-center lg:justify-start">
                <a href="#catalogo" className="px-10 py-5 bg-brand-primary hover:bg-orange-600 text-white font-black rounded-2xl shadow-xl shadow-orange-500/40 transition-all hover:-translate-y-1 active:scale-95">
                  EXPLORAR CATÁLOGO
                </a>
              </div>
            </div>
      ...
      <a
        ...
        target="_blank"
        className="fixed bottom-8 right-8 z-50 bg-[#25D366] text-white p-5 rounded-full shadow-[0_20px_50px_-10px_rgba(37,211,102,0.5)] hover:scale-110 transition-all duration-300 group active:scale-95 animate-bounce"
      >
      ...
      <footer className="bg-slate-950 text-white pt-24 pb-12 px-4 md:px-12 relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-20">
          <div className="space-y-8">
            <div className="flex items-center gap-4">
              ...
              <h4 className="text-3xl font-black tracking-tighter">BODEGA <span className="text-brand-primary">JIVA</span></h4>
            </div>
          ...
          <div className="space-y-8">
            <h5 className="text-sm font-black uppercase tracking-[0.3em] text-brand-primary">Contacto Directo</h5>
            <div className="space-y-6">
              <div className="flex items-center gap-4 group cursor-pointer">
                <div className="w-12 h-12 bg-blue-500/10 rounded-xl flex items-center justify-center text-blue-400 border border-blue-500/20 group-hover:bg-blue-500 group-hover:text-white transition-all">
                  <Phone size={20} />
                </div>
                <div>
                  <p className="text-xs text-slate-500 font-bold uppercase tracking-widest">Llámanos</p>
                  <p className="font-bold text-lg">+51 990 933 915</p>
                </div>
              </div>
              <div className="flex items-center gap-4 group cursor-pointer">
                <div className="w-12 h-12 bg-orange-500/10 rounded-xl flex items-center justify-center text-orange-400 border border-orange-500/20 group-hover:bg-orange-500 group-hover:text-white transition-all">
                  <MapPin size={20} />
                </div>
                <div>
                  <p className="text-xs text-slate-500 font-bold uppercase tracking-widest">Ubicación</p>
                  <p className="font-bold text-lg">Pro, SMP, Lima</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-8">
            <h5 className="text-sm font-black uppercase tracking-[0.3em] text-brand-primary">Horario de Atención</h5>
            <div className="p-8 bg-white/5 rounded-[2.5rem] border border-white/10 relative overflow-hidden group">
              ...
              <p className="text-slate-400 font-bold mb-2 text-xs uppercase tracking-widest">Lunes a Domingo</p>
              <p className="text-3xl font-black text-white tracking-tighter">8:00 AM - 12:00 AM</p>
              <div className="mt-6 inline-flex items-center gap-2 px-4 py-2 bg-green-500/10 text-green-400 rounded-full text-[10px] font-black uppercase tracking-widest border border-green-500/20">
